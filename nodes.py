...
class DownloadAndLoadMimicMotionModel:
    ...
    def loadmodel(self, precision, model):
        ...
        if not os.path.exists(model_path):
            ...
            snapshot_download(
                repo_id="Kijai/MimicMotion_pruned",
        ...
        unet_config = UNetSpatioTemporalConditionModel.load_config(
            os.path.join(script_directory, "configs", "unet_config.json")
        )
        print("Loading UNET")
        ...
        if is_accelerate_available:
            for key in sd:
                set_module_tensor_to_device(
                    self.unet, key, dtype=dtype, device=device, value=sd[key]
                )
        else:
        ...
        self.vae = (
            AutoencoderKLTemporalDecoder.from_pretrained(
                svd_path, subfolder="vae", variant="fp16", low_cpu_mem_usage=True
            )
            .to(dtype)
            .to(device)
            .eval()
        ...
        self.noise_scheduler = EulerDiscreteScheduler.from_pretrained(
            svd_path, subfolder="scheduler"
        )
        self.feature_extractor = CLIPImageProcessor.from_pretrained(
            svd_path, subfolder="feature_extractor"
        )

        print("Loading POSE_NET")
        self.pose_net = (
            PoseNet(noise_latent_channels=self.unet.config.block_out_channels[0])
            .to(dtype)
            .to(device)
            .eval()
        )
        pose_net_sd = comfy.utils.load_torch_file(
            os.path.join(
                script_directory, "models", "mimic_motion_pose_net.safetensors"
            )
        )
        ...
        mimic_model = {"pipeline": pipeline, "dtype": dtype}
        pbar.update(1)
        return (mimic_model,)
...
class DiffusersScheduler:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "scheduler": (["EulerDiscreteScheduler", "AnimateLCM_SVD"],),
                "sigma_min": (
                    "FLOAT",
                    {"default": 0.002, "min": 0.0, "max": 700.0, "step": 0.001},
                ),
                "sigma_max": (
                    "FLOAT",
                    {"default": 700.0, "min": 0.0, "max": 700.0, "step": 0.001},
                ),
...
class MimicMotionSampler:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "cfg_min": (
                    "FLOAT",
                    {"default": 2.0, "min": 0.0, "max": 20.0, "step": 0.01},
                ),
                "cfg_max": (
                    "FLOAT",
                    {"default": 2.0, "min": 0.0, "max": 20.0, "step": 0.01},
                ...
                "context_overlap": (
                    "INT",
                    {"default": 6, "min": 1, "max": 128, "step": 1},
                ),
    ...
    def process(
        ...
        context_size,
        context_overlap,
        optional_scheduler=None,
        pose_strength=1.0,
        image_embed_strength=1.0,
...
class MimicMotionDecode:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "decode_chunk_size": (
                    "INT",
                    {"default": 4, "min": 1, "max": 200, "step": 1},
                ),
            },
        }
    ...
    def process(self, mimic_pipeline, samples, decode_chunk_size):
        ...
        try:
            frames = pipeline.decode_latents(
                samples["samples"], num_frames, decode_chunk_size
            )
        except:
...
class MimicMotionGetPoses:
    ...
    def process(self, ref_image, pose_images, include_body, include_hand, include_face):
        ...
        download_path = os.path.join(folder_paths.models_dir, "mimicmotion")
        model_base_path = os.path.join(download_path, "DWPose")
